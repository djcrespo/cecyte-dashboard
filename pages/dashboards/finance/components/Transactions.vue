        ...
        <b-table-simple hover borderless responsive class="table-nowrap align-middle mb-0">
          ...
          <b-tbody>
            <b-tr v-for="(item, idx) in tableData" :key="idx">
              ...
              <b-td>
                <b-badge :variant="null" class="p-1" :class="item.status === 'success' ? 'bg-success-subtle text-success' : item.status === 'cancelled' ? 'bg-info-subtle text-info' : item.status === 'on-hold' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger'">
...
import { transactionsTableData } from '~/pages/dashboards/finance/components/data'
